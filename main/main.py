...
class Quiz(Frame):
    ...
    def __init__(self, questions, port):
        ...
        self.serial = serial
        self.read = serial.Serial(port, 9600)
   ...
   # caso contrário, mostra a próxima pergunta.
    ...
    def changePosition(self):
        valor = self.read.readline().decode('ascii').strip()
        # select = self.opt_selected.get()
        if (valor == "p"):
            self.next_btn()
            print("passou")
        if (valor == "c"):
   ...
   # cada uma das opções para a questão atual do botão de rádio.
    ...
    def display_options(self):
      ...
      # texto dos botões de rádio.
        for option in Optional[self.q_no]:
    ...
    def radio_buttons(self, questionOptions):
      # inicializar a lista com uma lista vazia de opções
        ...
        for option in questionOptions:
            ...
            srcImage = srcImage.resize((40, 40))
            # srcImage = Image.ANTIALIAS
            ...
            radio_btn = Radiobutton(gui, text=option["title"], variable=self.opt_selected,
                                    value=len(q_list)+1, font=("ariel", 14), indicatoron=0)
...
# serArduino = serial.Serial('COM2', 9600)
